...
export default function Explosivos() {
  ...
  const loadDatosIniciales = async () => {
    if (!faenaSeleccionada) return;

    setLoading(true);
    try {
      // Cargar polvorín de la faena
      const polvorinRes = await explosivosService.getPolvorinPorFaena(faenaSeleccionada);
      // Si tiene 'id', es el polvorín directamente. Si no, es {mensaje, polvorin: null}
      const polvorinData = polvorinRes?.id ? polvorinRes : null;
  ...
  const renderVista = () => {
    if (!polvorin && vistaActual !== 'configuracion') {
      return (
  ...
  // Mostrar mensaje si no hay faena seleccionada
  if (!faenaSeleccionada) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
        <Header />
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <Breadcrumb
            items={[
              { label: 'Dashboard Central', href: 'http://localhost:5173', icon: HiHome },
              { label: 'Explosivos - Polvorín' }
            ]}
          />
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            {polvorin && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className={`rounded-lg p-3 text-center ${stats.alertasTotal > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
